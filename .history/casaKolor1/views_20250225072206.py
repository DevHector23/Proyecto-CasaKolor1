...
from django.shortcuts import render, redirect
from .forms import SugerenciaForm  # Asegúrate de tener un formulario para esto

def enviar_sugerencia(request):
    if request.method == "POST":
        form = SugerenciaForm(request.POST)
        if form.is_valid():
            sugerencia = form.cleaned_data['sugerencia']  # Ajusta según los campos del formulario
            ...
            destinatario = "hector3208609853@gmail.com"  # Cambia esto al correo que recibirá las sugerencias
            send_mail(asunto, mensaje,EMAIL_HOST_USER, [destinatario])

            return redirect('sugerencia_exitosa')  # Redirige a la página de agradecimiento
    else:
        form = SugerenciaForm()
