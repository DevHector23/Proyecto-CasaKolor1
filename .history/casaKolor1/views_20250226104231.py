...
def finalizar_compra(request):
    if request.method == 'POST':
        # Restablecer la variable de sesión al comenzar una nueva compra
        request.session.pop('compra_procesada', None)
        
        form = PedidoForm(request.POST, request.FILES)
        if form.is_valid():
            # Marcar la sesión como procesada
            request.session['compra_procesada'] = True  
            ...
            if not request.session.get('correo_enviado', False):
                ...
                # Marcar que el correo ya fue enviado
                request.session['correo_enviado'] = True

            # Limpiar la variable de sesión para permitir futuras compras
            request.session.pop('compra_procesada', None)
