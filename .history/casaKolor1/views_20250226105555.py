...
def finalizar_compra(request):
    if request.method == 'POST':
        # Extraer el token de la transacción del formulario
        transaction_token = request.POST.get('transaction_token')
        
        # Verificar si este token ya fue procesado
        cache_key = f'processed_order_{transaction_token}'
        if cache.get(cache_key):
            # Esta transacción ya fue procesada, no hacer nada
            return JsonResponse({'success': True, 'message': 'Pedido ya procesado anteriormente'})
            
        # Marcar esta transacción como en proceso
        cache.set(cache_key, 'processing', 30)  # 30 segundos de expiración
        
        # Usar una transacción de base de datos para garantizar que todo se guarda o nada
        with transaction.atomic():
            ...
            if form.is_valid():
                ...
                pedido.transaction_id = transaction_token  # Guardar el token para referencia
                pedido.save()
                ...
                # Marcar esta transacción como completada (30 minutos de expiración)
                ...
                # Eliminar la marca en caso de error
