...
def finalizar_compra(request):
    if request.method == 'POST':
        ...
        email_key = f'email_sent_{transaction_token}'
        ...
        if form.is_valid():
            try:
                # Crear el pedido y sus detalles en una transacción atómica
                ...
                # Verificar si ya se envió un correo para esta orden
                if not cache.get(email_key):
                    # Marcar que se está enviando el correo
                    cache.set(email_key, True, 60 * 30)  # 30 minutos
                    ...
                    try:
                        ...
                        email.send(fail_silently=False)  # Cambiado a False para detectar errores
                        logger.info(f"Correo enviado exitosamente para el pedido: {pedido.id}")
                    except Exception as e:
                        logger.error(f"Error al enviar correo: {e}")
                        # Eliminar la marca de correo enviado para que se pueda intentar de nuevo
                        cache.delete(email_key)
                
                # Responder al cliente después de completar todo el procesamiento
                return JsonResponse({'success': True, 'message': 'Pedido creado correctamente'})
                    
            except Exception as e:
                logger.error(f"Error al procesar el pedido: {e}")
                # Eliminar las marcas de caché en caso de error
