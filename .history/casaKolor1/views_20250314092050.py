                ...
                with transaction.atomic():
                    ...
                    try:
                        email_usuario = EmailMultiAlternatives(
                            ...
                            [pedido.correo]  # Correo del usuario
                        )
                        
                        email_usuario.attach_alternative(html_content, "text/html")
                        
                        if hasattr(pedido, 'comprobante') and pedido.comprobante:
                            email_usuario.attach_file(pedido.comprobante.path)
                        
                        email_usuario.send(fail_silently=True)  # Enviar sin arrojar excepciones
                    except Exception as e:
                        logger.error(f"Error al enviar correo al usuario: {e}")

                    # Enviar el correo a la empresa
                    try:
                        email_empresa = EmailMultiAlternatives(
                            "Nuevo Pedido Recibido - CasaKolor1",  # Asunto diferente para la empresa
                            f"Se ha recibido un nuevo pedido de {pedido.nombre} ({pedido.correo}).\n\nDetalles del pedido:\n\n{text_content}",  # Contenido simplificado para la empresa
                            settings.EMAIL_HOST_USER,
                            ['hector3208609853@gmail.com']  # Correo de la empresa
                        )
                        
                        if hasattr(pedido, 'comprobante') and pedido.comprobante:
                            email_empresa.attach_file(pedido.comprobante.path)
                        
                        email_empresa.send(fail_silently=True)  # Enviar sin arrojar excepciones
                    except Exception as e:
