    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if User.objects.filter(username=username).exists():
                ...
                messages.error(request, f'El usuario {username} ya existe. Por favor, elige otro nombre de usuario.')
                # Mantener los datos del formulario para que no se borren
                register_form = form
                # Renderizar de nuevo la página de registro con el formulario y el mensaje
                return render(request, 'login.html', {
                    'form': login_form,
                    'register_form': register_form,
                    'mode': 'register'  # Asegurarse de que se muestre la sección de registro
                })
            else:
