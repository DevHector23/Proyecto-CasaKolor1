...
def lista_herramientas(request):
    try:
        # 1. Obtener herramientas con manejo de errores
        herramientas = productos.objects.filter(categoria='herramientas')
        
        # 2. Sanitizar los datos problemáticos
        herramientas_saneadas = []
        for herramienta in herramientas:
            try:
                # Crear copia segura de los datos
                herramienta_saneada = {
                    'id': herramienta.id,
                    'nombre': force_str(herramienta.nombre),
                    'descripcion': force_str(herramienta.descripcion),
                    'precio': herramienta.precio,
                    'imagen': herramienta.imagen,
                    'categoria': herramienta.categoria
                }
                herramientas_saneadas.append(herramienta_saneada)
                
            except UnicodeError as e:
                logger.error(f"Error decodificando herramienta ID {herramienta.id}: {str(e)}")
                continue
        
        # 3. Renderizar con contexto seguro
        return render(request, 'herramientas.html', {
            'productos': herramientas_saneadas,
            'error_decodificacion': len(herramientas) != len(herramientas_saneadas)
        })
        
    except Exception as e:
        logger.critical(f"Error crítico en lista_herramientas: {str(e)}")
        return render(request, 'error.html', {
            'mensaje': 'Error al cargar las herramientas. Por favor intente más tarde.'
        }, status=500)
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Asociar el usuario actual al pedido si está autenticado
