...
def finalizar_compra(request):
    # Verificar si el usuario está autenticado (aunque permitimos compras sin registro)
    user = request.user if request.user.is_authenticated else None
    
    # Generar o obtener token de transacción
    ...
    if cache.get(cache_key):
        logger.warning(f"Intento de re-procesar transacción {transaction_token}")
        return JsonResponse({
            'success': False, 
            'message': 'Esta transacción ya fue procesada anteriormente'
        })
    
    # Bloquear la transacción para evitar duplicados
    cache.set(cache_key, True, 300)  # 5 minutos de bloqueo
    ...
    try:
        with transaction.atomic():
            # Verificar nuevamente si la transacción ya existe
            if Pedido.objects.filter(transaction_id=transaction_token).exists():
                return JsonResponse({
                    'success': False,
                    'message': 'Esta transacción ya fue registrada'
                })
            
            # Crear el pedido
            pedido = form.save(commit=False)
            pedido.total = Decimal(request.POST.get('total', 0))
            pedido.transaction_id = transaction_token
            pedido.usuario = user  # Asignar usuario (puede ser None)
            ...
            for item in items:
                try:
                    producto = productos.objects.get(id=item.get('id'))
                    ...
                    # Aquí podrías añadir lógica para reducir el stock si es necesario
                    # producto.stock -= cantidad
                    # producto.save()
                    
                except productos.DoesNotExist:
                    logger.error(f"Producto ID {item.get('id')} no encontrado")
                    continue
                except (ValueError, TypeError) as e:
                    logger.error(f"Error en datos del item: {str(e)}")
                    continue
            
            # Extender el bloqueo de cache por 30 minutos más
            cache.set(cache_key, True, 60 * 30)
            
            # Enviar emails de confirmación (solo si el pedido se creó correctamente)
            email_result = enviar_confirmacion_email_interno(pedido)
            if email_result.get('success'):
                logger.info(f"Emails enviados para pedido {pedido.id}")
            else:
                logger.warning(f"Error enviando emails: {email_result.get('message')}")
            
            # Limpiar carrito si el usuario está autenticado
            if user:
                try:
                    # Asumiendo que tienes un modelo Carrito relacionado al usuario
                    Carrito.objects.filter(usuario=user).delete()
                    logger.info(f"Carrito limpiado para usuario {user.id}")
            ...
            logger.error(f"Error en finalizar_compra: {str(e)}", exc_info=True)
            cache.delete(cache_key)
                return JsonResponse({
            'success': False,
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            reset_link = request.build_absolute_uri(
                f'/cambiar-contrasena/{uid}/{token}/'
            )
